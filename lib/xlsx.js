var alphabet, dateFormat, formatDate, getAfterDays;
...
alphabet = function(i) {
  ...
  t = Math.floor(i / 26) - 1;
  return (t > -1 ? alphabet(t) : "") + s.charAt(i % 26);
};
...
exports.decode = function(file) {
  var disp, f, fid, fmt, format, i, id, idx, j, k, l, numFmts, processTime, result, s, sharedStrings, sid, styles, t, v, w, zip, zipTime, _i, _len;
  zipTime = +new Date();
  ...
  if (s[0].indexOf("<numFmts ") > 0) {
    ...
    for (_i = 0, _len = numFmts.length; _i < _len; _i++) {
      f = numFmts[_i];
  ...
  while (i--) {
    ...
    while (--j) {
      ...
      while (--l) {
        ...
        disp = "";
        if (t.indexOf("<v>") >= 0) {
          disp = t.substring(t.indexOf("<v>") + 3, t.indexOf("</v>"));
        }
        if (disp !== "") {
          if (t.indexOf('t="s"') > 0) {
            idx = -1;
            ...
            if (sid >= 0) {
              fmt = "GENERAL";
              if (sid >= 0 && styles[sid]) {
                fmt = styles[sid]["format"];
              }
              if (fmt.indexOf('YY') >= 0 || fmt.indexOf('M') >= 0 || fmt.indexOf('D') >= 0) {
...
exports.encode = function(file) {
  var contentTypes, dateStr, docProps, fmt, i, id, index, j, k, l, processTime, props, result, s, sharedStrings, t, w, worksheet, worksheets, xl, xlRels, xlWorksheets, zip, zipTime;
  zip = new JSZip();
  contentTypes = [[], []];
  props = [];
  xlRels = [];
  ...
  while (w--) {
    ...
    while (++i < l) {
      ...
      while (++j < k) {
        ...
        if (t instanceof Date) {
          ...
          if (index < 0) {
            index = sharedStrings[0].push(dateStr) - 1;
          }
          s += "<c r=\"" + alphabet(j) + (i + 1) + "\" t=\"s\"><v>" + index + "</v></c>";
        } else if (t && isNaN(parseFloat(t)) || !isFinite(t)) {
          sharedStrings[1]++;
    ...
    if (worksheet.table) {
      ...
      while (++i < l) {
        s += "<tableColumn id=\"" + (i + 1) + "\" name=\"" + worksheet[0][i] + "\"/>";
      }
