...
(function() {
  var START_DAY, alphabet, col2num, convertDate, escape, getAttr, numFmts, ref2cr, typeOf, unescape;
  ...
  exports.decode = function(file) {
    ...
    if (zip.files["xl/sharedStrings.xml"]) {
      ...
      i = s.length;
      while (--i) {
        sharedStrings[i - 1] = unescape(s[i].substring(0, s[i].indexOf("<")));
      }
    }
  ...
  exports.encode = function(file) {
    ...
    while (w--) {
      ...
      while (++i < l) {
        ...
        while (++j < k) {
          ...
          if (val && typeof val === "string" && isNaN(parseFloat(val))) {
            ...
            if (index < 0) {
              index = sharedStrings[0].push(escape(val)) - 1;
            }
            val = index;
            t = "s";
      ...
      worksheets.unshift("<sheet name=\"" + escape(worksheet.name || "Sheet" + id) + "\" sheetId=\"" + id + "\" r:id=\"rId" + id + "\"/>");
    }
    i = styles.length;
    t = [];
    while (--i) {
