...
numAlpha = (i) ->
  t = Math.floor(i / 26) - 1
  ((if t > -1 then numAlpha(t) else "")) + alphabet.charAt(i % 26)
...
escapeXML = (s) ->
  (s or "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace /'/g, "&#x27;"
...
exports.decode = (file) ->    
  ...
  result.data.push s.data  for s in result.worksheets
...
exports.encode = (file) ->
  ...
  while w--
    ...
    while ++i < l
      ...
      while ++j < k
        ...
        style =
          ...
          formatCode: cell.formatCode or "General"

        colWidth = 0
        if val and typeof val is "string" and not isFinite(val)
          ...
          val = convertDate(val)
          style.formatCode = cell.formatCode or "mm-dd-yy"
          colWidth = val.length
        ...
        unless columns[j]
          columns[j] =
            autoWidth: false
            max: 0
        columns[j].autoWidth = true  if cell.autoWidth
        columns[j].max = colWidth  if colWidth > columns[j].max
        ...
        if cell.rowSpan > 1 or cell.colSpan > 1
          ...
          delete cell.colSpan
        s += "<c r=\"" + numAlpha(j) + (i + 1) + "\"" + ((if style then " s=\"" + style + "\"" else "")) + ((if t then " t=\"" + t + "\"" else ""))
        if val isnt null
          s += ">" + ((if cell.formula then "<f>" + cell.formula + "</f>" else "")) + "<v>" + val + "</v></c>"
        else
          s += "/>"
      s += "</row>"
    cols = []
    ...
    if worksheet.table
      ...
      s += "<tableColumn id=\"" + (i + 1) + "\" name=\"" + ((if data[0][i].hasOwnProperty("value") then data[0][i].value else data[0][i])) + "\"/>"  while ++i < l
  ...
  while --i
    ...
    if style.formatCode isnt "General"
    ...
    if style.borders
      ...
      for edge of { left: 0, right: 0, top: 0, bottom: 0, diagonal: 0 }
        if style.borders[edge]
          color = style.borders[edge]
            
          # add transparency if missing
          color = "FF" + color  if color.length is 6
      ...
      borderIndex = borders.push(border) - 1  if borderIndex < 0
      
    # font declaration: add a new declaration and refer to it in style
    fontIndex = 0
    if style.bold or style.italic or style.fontSize or style.fontName
      font = ["<font>"]
      font.push "<b/>"  if style.bold
      font.push "<i/>"  if style.italic
      font.push "<sz val=\"", style.fontSize or defaultFontSize, "\"/>"
      font.push "<color theme=\"1\"/>"
      font.push "<name val=\"", style.fontName or defaultFontName, "\"/>"
      font.push "<family val=\"2\"/>", "</font>"
      font = font.join("")
        
      # try to reuse existing font
      fontIndex = fonts.indexOf(font)
      fontIndex = fonts.push(font) - 1  if fontIndex < 0
      
    # declares style, and refer to optionnal formatCode, font and borders
    styles[i] = ["<xf xfId=\"0\" fillId=\"0\" borderId=\"", borderIndex, "\" fontId=\"", fontIndex, "\" numFmtId=\"", style.formatCode, "\" ", ((if style.hAlign or style.vAlign then "applyAlignment=\"1\" " else " ")), ((if style.formatCode > 0 then "applyNumberFormat=\"1\" " else " ")), ((if borderIndex > 0 then "applyBorder=\"1\" " else " ")), ((if fontIndex > 0 then "applyFont=\"1\" " else " ")), ">"]
    if style.hAlign or style.vAlign
      styles[i].push "<alignment"
      styles[i].push " horizontal=\"", style.hAlign, "\""  if style.hAlign
      styles[i].push " vertical=\"", style.vAlign, "\""  if style.vAlign
      styles[i].push "/>"
    styles[i].push "</xf>"
...
# NodeJs export
#module.exports = encode  if typeof exports is "object" and typeof module is "object"
#module.exports = decode  if typeof exports is "object" and typeof module is "object"
#module.exports = xlsx  if typeof exports is "object" and typeof module is "object"
