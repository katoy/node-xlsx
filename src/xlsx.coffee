...
exports.decode = (file) -> # v1.1.0
  zipTime = +new Date()
  zip = new JSZip().load(file,
    base64: true
  )
  result =
    worksheets: []
    zipTime: +new Date() - zipTime
  ...
  #{ Process style
  styles = []
  s = zip.files["xl/styles.xml"].data.split("<t>")
  if s[0].indexOf("<numFmts ") > 0
    numFmts = s[0].substring(s[0].indexOf("<numFmts "))
    #console.log "-------#{numFmts}"
    numFmts = numFmts.substring(0, numFmts.indexOf("</numFmts>")) + "</numFmts>"
    numFmts = numFmts.split("<numFmt ")
    # console.log "-------#{f}" for f in numFmts
    for f in numFmts
      t = f.substring(f.indexOf('numFmtId="') + 10)
      t = t.substring(0, t.indexOf('"'))
      fid = parseInt(t, 10)

      t = f.substring(f.indexOf('formatCode="') + 12)
      t = t.substring(0, t.indexOf('"'))
      format = t
      # console.log "#{formatid}:[#{format}]"
      if fid >= 0
        v = {
          fid: fid
          format: format
  ...
  while i--
    ...
    while --j # Don't process j === 0, because s[0] is the text before the first row element
      ...
      while --l # Don't process l === 0, because k[0] is the text before the first c (cell) element
        t = k[l]
        disp = ""
        disp = t.substring(t.indexOf("<v>") + 3, t.indexOf("</v>")) if t.indexOf("<v>") >= 0
        if disp != ""
          if t.indexOf('t="s"') > 0
            idx = -1
            idx = parseInt(disp, 10) if idx
            disp = sharedStrings[idx] if idx >= 0
          else if t.indexOf('s="') > 0
            sid = t.substring(t.indexOf('s="') + 3)
            sid = sid.substring(0, t.indexOf('"'))
            sid = parseInt(sid, 10) - 1
            if sid >= 0
              fmt = "GENERAL"
              fmt = styles[sid]["format"] if sid >= 0 and styles[sid]
              # console.log "------ #{fmt}, #{disp}"
              if fmt.indexOf('YY') >= 0 or fmt.indexOf('M') >= 0 or fmt.indexOf('D') >= 0
                disp = formatDate(getAfterDays(parseInt(disp, 10)), fmt)
        w[0].unshift(disp)
  #}
...
exports.encode = (file) -> # v1.1.0
  ...
  while w-- # Generate worksheet (gather sharedStrings), and possibly table files, then generate entries for constant files below
    ...
    while ++i < l
      ...
      while ++j < k
        ...
        if t instanceof Date
          console.log t
          days = Math.ceil((t.getTime() - (new Date("1900-01-01")).getTime()) / 86400000)
          s += "<c r=\"" + alphabet(j) + (i + 1) + "\" s=\"1\" t=\"n\"><v>#{days}</v></c>"
          # fmt = "yyyy-mm-dd"
          # dateStr = dateFormat(t, fmt)
          # sharedStrings[1]++ # Increment total count, unique count derived from sharedStrings[0].length
          # index = sharedStrings[0].indexOf(dateStr)
          # index = sharedStrings[0].push(dateStr) - 1  if index < 0
          #s += "<c r=\"" + alphabet(j) + (i + 1) + "\" t=\"s\"><v>" + index + "</v></c>"
        else if t and isNaN(parseFloat(t)) or not isFinite(t) # If value is text, place a sharedString reference instead of the value (use parseInt because +emptyString === 0)
          sharedStrings[1]++ # Increment total count, unique count derived from sharedStrings[0].length
          index = sharedStrings[0].indexOf(t)
          index = sharedStrings[0].push(t) - 1  if index < 0
          s += "<c r=\"" + alphabet(j) + (i + 1) + "\" t=\"s\"><v>" + index + "</v></c>"
        else
          s += "<c r=\"" + alphabet(j) + (i + 1) + "\"><v>" + t + "</v></c>"
      s += "</row>"
    ...
    if worksheet.table
      ...
      s += "<tableColumn id=\"" + (i + 1) + "\" name=\"" + worksheet[0][i] + "\"/>"  while ++i < l
